...
%       psiWeights 
%           formID = 1 (n_modes x 1) -> weighting factor for MAC value
%           formID = 2 (n_meas x n_modes) -> weighting factor for each
%             entry of eigenvector 
%       WARNING: if using formID = 2, but the dimension of psiWeights is
%         n_modes x 1, all the entries of the same eigenvector will have
%         the same weighing factor.
...
if(updatingOpts.formID < 3 || updatingOpts.formID >= 4)
    n_x = n_alpha;
...
structModel.K_j = structModel.K_j(orderIndex, orderIndex, :);

if( updatingOpts.formID < 2.3 || updatingOpts.formID >= 3)
...
fun = @(x) ModelUpdatingObjJac(x, structModel, expModes, updatingOpts,optimzOpts.toolBox);
