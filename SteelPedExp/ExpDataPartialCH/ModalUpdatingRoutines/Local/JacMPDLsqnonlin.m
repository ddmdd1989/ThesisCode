...
for i = 1 : n_modes
    for j = 1 : n_alpha
        ...
        if (normOpt == 1)
            ...
            [~, q_i] = max(abs( simModes.psi_m(:,i) ));
            P_i = setdiff(1 : N, q_i);
            Q_i = P_i(1 : n_meas - 1);
                        
            B_m = B(:,Q_i);
            B_q = B(:,q_i);
            B_u = B(:,expModes.n_meas  + 1: end);
            
            coeff = simModes.psi_m(Q_i,i)' / simModes.psi_m(q_i,i);
            B_prim = [B_m - B_q * coeff B_u];
            dPsi_dAlpha_j(P_i,1) = B_prim \ b;
            dPsi_dAlpha_j(q_i) = - coeff * dPsi_dAlpha_j(Q_i);
            dPsi_m(:, j, i) = dPsi_dAlpha_j(1 : expModes.n_meas);
...
for i = 1 : n_modes
    ...
    if (objOpt == 1)
        ...
        d_ri_MAC = (-expModes.psiWeights(i) / sqrt(MACValue)) * ...
            ...
            * dPsi_m(expModes.good_ch{i}, :, i);
        jac = [jac; d_ri_eigFreqTerm(i,:); d_ri_MAC];
    else
        ...
        if(normOpt == 1)
            ...
            d_ri_psi = -dPsi_m(expModes.good_ch{i}(Q_i), :, i) .* (expModes.psiWeights(expModes.good_ch{i}(Q_i),i) * ones(1,n_alpha)) ;
            jac = [jac; d_ri_eigFreqTerm(i,:); d_ri_psi];
        else
            d_ri_psi = -expModes.psiWeights(i) * ...
                (psiExp' * psiSim* eye(expModes.n_meas) + psiSim *...
                psiExp') * dPsi_m(:, :, i);
