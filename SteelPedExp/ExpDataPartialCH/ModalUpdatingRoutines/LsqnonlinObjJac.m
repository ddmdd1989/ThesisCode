...
if ( updatingOpts.formID ~= 3 )
    ...
    if ( updatingOpts.modeMatch == 1 )
        for i = 1 : expModes.n_modes
            ...
            psi_exp_temp = expModes.psiExp(expModes.good_ch{i},i);
            [~, matchedModeIndex(i)] = max(MAC(psi_exp_temp, psiTemp) );
        end
...
% if(~isempty( find(x > updatingOpts.x_ub * 1.5, 1)) || ...
%         ~isempty(find(x < updatingOpts.x_lb * 1.5, 1)))
%     r = zeros(length(r),1);
%     jac = zeros(length(r),length(x));
% end
end
