...
%
% function [r, jac] = ModelUpdatingObjJac(x, structModel, expModes, updatingOpts)
%
% function [r] = ModelUpdatingObjJac(x, structModel, expModes, updatingOpts)
%
...
if ( updatingOpts.formID ~= 3 )
    ...
    if ( updatingOpts.modeMatch == 1 )
        for i = 1 : expModes.n_modes
            psiTemp = psi_m(expModes.good_ch{i},:);
            psiExpTemp = expModes.psiExp(expModes.good_ch{i},i);
            [~, matchedModeIndex(i)] = max(MAC(psiExpTemp, psiTemp) );
        end
...
if nargout > 1
    ...
    jac_reg = updatingOpts.reg * eye(length(x));
    jac = [jac; jac_reg];
end
