function [M_C, K_C, P_K, P_M] = StrucCondensAllMass(M, K, P_K, P_M,  K_al, M_al, M_C, n_s, n_i, n_r, n_q, condensationType)
...
for i = 1 : n_i + n_q
    P_M(n_s+1:N_tilde,n_s+1:N_tilde,n_m+i) = MR_diag(i,i)*VR_l(:,i)*VR_l(:,i)' ;
end


% Condensed mass and stiffness matrics M_C and K_C

M_C = M_C+ sum(P_M(:,:,2:n_i + n_q+1),3) ;    
K_C = sum(P_K,3) ;
