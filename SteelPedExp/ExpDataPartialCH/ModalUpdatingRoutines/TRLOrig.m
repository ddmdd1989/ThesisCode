function [x, RESNORM, OUTPUT] = TRLOrig(rFun,Jacobian,Hessian,x0,options)

% function [x, RESNORM, RESIDUAL, EXITFLAG, OUTPUT] = TRL_original(rFun,Jacobian,Hessian,x0,options)
%
...
%           2: TolX termination criterion           3: TolFun termination criterion
...
Delta = 0.1;       % Initial value for trust region radius.
...
for k = 1:maxNumItr,
    ...
    % Full step to trust-region boundary
    P_s = - Delta * gk / norm(gk);
    gHg = gk' * Hk * gk;
    if(gHg <= 0)
        tau = 1;
    else
        tau = min(1, norm(gk) ^ 3 / Delta / gHg);
    end
    % Actual step within bound
    P_c = tau * P_s;
    % Estimated function reduction 
    predred = -(P_c' * gk + 0.5 * P_c' * Hk * P_c);
