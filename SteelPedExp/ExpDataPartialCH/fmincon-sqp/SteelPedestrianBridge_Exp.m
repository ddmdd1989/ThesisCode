...
if(strcmp(optimzOpts.toolBox,'L_M'))
    filename = ['Space_form' num2str(updatingOpts.formID) '_JAC' ...
        optimzOpts.gradSel '_reg' num2str(updatingOpts.reg) '_LMOwn.mat'];
elseif(strcmp(optimzOpts.toolBox,'Gauss-Newton'))
    filename = ['Space_form' num2str(updatingOpts.formID) '_JAC' ...
        optimzOpts.gradSel '_reg' num2str(updatingOpts.reg) '_GSNT.mat'];
elseif(strcmp(optimzOpts.toolBox,'lsqnonlin'))
    if(strcmp(optimzOpts.optAlgorithm,'Levenberg-Marquardt'))
        filename = ['Space_form' num2str(updatingOpts.formID) '_JAC' ...
            optimzOpts.gradSel '_reg' num2str(updatingOpts.reg) '_LM.mat'];
    elseif(strcmp(optimzOpts.optAlgorithm,'trust-region-reflective'))
        filename = ['Space_form' num2str(updatingOpts.formID) '_JAC' ...
            optimzOpts.gradSel '_reg' num2str(updatingOpts.reg) '_TRR.mat'];
    end
elseif(strcmp(optimzOpts.toolBox,'fmincon'))
    filename = ['Space_form' num2str(updatingOpts.formID) '_JAC' ...
        optimzOpts.gradSel '_reg' num2str(updatingOpts.reg) optimzOpts.optAlgorithm '.mat'];
end
