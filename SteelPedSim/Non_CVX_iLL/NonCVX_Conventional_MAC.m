...
[psi_e,Lambda_e] = eigs(K_act,M0,20,'sm');
[Lambda_e,dummyInd] = sort((diag(Lambda_e)),'ascend') ;

Lambda_e = Lambda_e(Modeindex);

psi_e = psi_e(:,dummyInd(Modeindex));

psi_m = psi_e(MDOF,:);


for i = 1:n_modes
    [~,expModes.q(i)] = max(abs(psi_m(:,i)));
    psi_m(:,i) = psi_m(:,i) / psi_m(expModes.q(i),i);
end

expModes.lambdaExp = Lambda_e;
expModes.psiExp = psi_m;
expModes.measDOFs = MDOF;
